...
- `NanothreeRendererAdapter` wraps nanothree — a lightweight pure-WebGPU renderer with Three.js-compatible API
  ...
  - Geometry: `BoxGeometry`, `SphereGeometry`, `CapsuleGeometry`, `CylinderGeometry`, `ConeGeometry`, `CircleGeometry`, `PlaneGeometry`, `TorusGeometry`, `TetrahedronGeometry` — all ported from Three.js with identical vertex output; all geometry types are wired in the adapter (cylinder and torus included)
  ...
  - GLTF/GLB model loading via `GLTFLoader`: parses binary GLB and JSON GLTF formats, extracts mesh geometry (positions, normals, UVs, indices), materials (base color factor + albedo texture), node hierarchy, and animation data (channels/samplers for translation, rotation, scale tracks); embedded and external textures supported; Draco mesh decompression via `KHR_draco_mesh_compression` extension; texture extensions `EXT_texture_webp`, `EXT_texture_avif`, `KHR_texture_basisu` (KTX2) supported; skeletal/skinned mesh animation via `Bone`/`Skeleton`/`SkinnedMesh` (GLTF `skins`, `JOINTS_0`, `WEIGHTS_0`); no morph targets
  - Draco decompression: `GLTFLoader.setDracoDecoderPath(path)` enables loading Draco-compressed models; uses WASM decoder from `library/third-party/draco-1.5.7/` (gltf variant); decoder is loaded lazily on first use and cached; in dev/editor mode served at `/__mana/draco/` via Vite middleware; in production copied to `draco/` in the build output; `getDracoDecoderPath()` from `assets.ts` returns the correct path for the current environment
  - KTX2/Basis textures: `GLTFLoader.setBasisTranscoderPath(path)` enables loading models with `KHR_texture_basisu` KTX2 textures; uses Basis Universal WASM transcoder from `library/third-party/basis-1.50/`; transcodes to RGBA8 on the main thread; served at `/__mana/basis/` in dev/editor, copied to `basis/` in production; `getBasisTranscoderPath()` from `assets.ts` returns the correct path
  ...
  - Animation: `AnimationClip`, `AnimationMixer`, `AnimationAction`, `KeyframeTrack` classes in `library/src/nanothree/animation.ts` — lightweight Three.js-compatible API for GLTF node-level transform animation (translation, rotation via quaternion slerp, scale); supports looping, clamping, crossfade between clips; `NanothreeAnimationHelper` manages per-entity mixers identical to ThreeAnimationHelper pattern; skeletal animation works via Bone nodes driven by these tracks; no morph targets
  - Skinned meshes: `Bone`, `Skeleton`, `SkinnedMesh` classes in `library/src/nanothree/skinned-mesh.ts` — Three.js-compatible API for GPU skeletal mesh deformation
    - `Bone` extends `Object3D` (`isBone = true`); `Skeleton` holds bones + inverse bind matrices + flat `boneMatrices: Float32Array`; `SkinnedMesh` extends `Object3D` (`isMesh + isSkinnedMesh = true`) with `bind(skeleton, bindMatrix?)`, `bindMatrix`/`bindMatrixInverse`
    - GPU: bone matrices in storage buffer (bind group 2); WGSL shaders for skinned solid, skinned textured, skinned shadow; vertex layout pos(3)+norm(3)+uv(2)+joints(4)+weights(4) = 64 bytes; vertex shader: `skinMatrix = Σ(weight × boneMatrix[joint])`
    - `BufferGeometry.skinIndices`/`skinWeights` (Float32Array, 4 per vertex); `Uint16BufferAttribute` for Three.js API compat; GLTF loader auto-creates Bone/Skeleton/SkinnedMesh from skin data; parses `JOINTS_0` (u8/u16→float) and `WEIGHTS_0`
    - Scene collects `SkinnedMesh` in `scene.skinnedMeshes[]`; renderer classifies into skinned-solid and skinned-textured; `_updateBoneMatrices()` bakes bind matrix (identity fast path)
  - Orthographic view: `PerspectiveCamera.orthoOverride` field switches projection to orthographic in `updateViewProjection()`; adapter implements `setOrthographicView()` with 6 standard views (front/back/left/right/top/bottom) using Blender-style numpad shortcuts (1/3/7); auto-returns to perspective when user orbits
